...
# The anomaly score for each interaction in 𝑒
  return Sc_list
...
def F_FADE(model, dataset, t_setup, W_upd, alpha, M, T_th, epochs, online_train_steps, max_batch_size, device):
  ...
  label = dataset.label
  data_idx = 0
  Sc = list() # liste vide (de score d'anomalies)
  ...
  for t in range(1, T_max + 1):
    ...
    if t == (t_setup + k * W_upd):
      ...
      k += 1
      Act_S = set()
#Liste de socre d'anomamlies des interactions 
  return Sc
